...
if [ "$1" == "import" ]; then
    ...
    sudo -u postgres createdb -E UTF8 -O postgres ${DJANGO_DB_NAME:'django_db_name'}
    sudo -u postgres psql -d tourist_archive -c "ALTER USER postgres WITH PASSWORD ${DJANGO_DB_PASSWORD:'postgres'};"
    sudo -u postgres psql -d tourist_archive -c "CREATE EXTENSION postgis;" 
    sudo -u postgres psql -d tourist_archive -c "CREATE EXTENSION hstore;"
